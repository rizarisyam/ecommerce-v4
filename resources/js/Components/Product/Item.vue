<template>
    <div class="py-6">
        <div class="flex max-w-md bg-white shadow-lg rounded-lg overflow-hidden">
            ...
            <div class="w-2/3 p-4">
                <h1 class="text-gray-900 font-bold text-2xl">{{ name ?? "Backpack" }}</h1>
                <p
                    class="mt-2 text-gray-600 text-sm"
                >{{ desc ?? "Lorem ipsum dolor sit amet consectetur adipisicing elit In odit exercitationem fuga id nam quia" }}</p>
                ...
                <div class="flex item-center justify-between mt-3">
                    <h1 class="text-gray-700 font-bold text-xl">{{ rupiahFormat(price) ?? "$220" }}</h1>
                    <button
